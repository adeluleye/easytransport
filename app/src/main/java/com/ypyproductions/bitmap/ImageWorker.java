...
//import com.wizcodegroup.prepa.object.PlacePhotoObject;
...
public abstract class ImageWorker {
	...
	protected AssetManager mAssestManager;
	...
	public void loadImageNoDisplayDefault(Object data, ImageView imageView) {
		if (data == null) {
			return;
		}
	...
	public static void cancelWork(ImageView imageView) {
		...
		if (bitmapWorkerTask != null) {
			...
			DBLog.d(TAG, "cancelWork - cancelled work for " + bitmapData);
		}
	}
	...
	public static boolean cancelPotentialWork(Object data, ImageView imageView) {
		...
		if (bitmapWorkerTask != null) {
			...
			if (bitmapData == null || !bitmapData.equals(data)) {
				bitmapWorkerTask.cancel(true);
				DBLog.d(TAG, "cancelPotentialWork - cancelled work for " + data);
			}
			else {
	...
	private class BitmapWorkerTask extends AsyncTask<Object, Void, BitmapDrawable> {
		...
		protected BitmapDrawable doInBackground(Object... params) {
			DBLog.d(TAG, "doInBackground - starting work");

			data = params[0];
			String mStrData="";
			if(data instanceof AbstractImageObject){
				mStrData=((AbstractImageObject)data).getLink();
			}
			else{
				mStrData = String.valueOf(data);
			}
			final String dataString = mStrData;
			...
			DBLog.d(TAG, "doInBackground - finished work");
		...
		protected void onPostExecute(BitmapDrawable value) {
			...
			/*if (value != null && imageView != null) {
				DBLog.d(TAG, "onPostExecute - setting bitmap");
				setImageDrawable(imageView, value);
				if(data instanceof PlacePhotoObject){
					DBLog.d(TAG, "=============================>checkitout");
					IDBCallback mCallback = ((AbstractImageObject)data).getFinishCallback();
					((AbstractImageObject)data).setBitmap(value);
					if(mCallback!=null){
						mCallback.onAction();
					}
				}
