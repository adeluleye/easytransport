...
public class ImageCache {
	...
	private void init(ImageCacheParams cacheParams) {
		...
		if (mCacheParams.memoryCacheEnabled) {
			DBLog.d(TAG, "Memory cache created (size = " + mCacheParams.memCacheSize + ")");
				...
				if (mCacheParams.diskCacheEnabled && diskCacheDir != null) {
					...
					if (getUsableSpace(diskCacheDir) > mCacheParams.diskCacheSize) {
						try {
							...
							DBLog.d(TAG, "Disk cache initialized");
						}
	...
	protected Bitmap getBitmapFromReusableSet(BitmapFactory.Options options) {
		...
		if (mReusableBitmaps != null && !mReusableBitmaps.isEmpty()) {
			...
			while (iterator.hasNext()) {
				SoftReference<Bitmap> mSoftReference =iterator.next();
				synchronized (mSoftReference) {
					item = mSoftReference.get();
				}
	...
	public void clearCache() {
		if (mMemoryCache != null) {
			...
			DBLog.d(TAG, "Memory cache cleared");
		}
		...
		synchronized (mDiskCacheLock) {
			...
			if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
				try {
					...
					DBLog.d(TAG, "Disk cache cleared");
				}
				catch (IOException e) {
					DBLog.e(TAG, "clearCache - " + e);
					e.printStackTrace();
				}
				mDiskLruCache = null;
				initDiskCache();
	...
	public void flush() {
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache != null) {
				try {
					mDiskLruCache.flush();
					DBLog.d(TAG, "Disk cache flushed");
				}
				catch (IOException e) {
					DBLog.e(TAG, "flush - " + e);
					e.printStackTrace();
